...
//    vector<double> service_time = {5, 3, 3, 5, 6, 3};
    ...
    Event next_arrival_time, current_departure_time;
    Event curr_event;
    ...
    while (true) {
        curr_event = min_event({current_departure_time, next_arrival_time});
        if (curr_event == INF)
            break;
        ...
        if (curr_event == current_departure_time) {
            // update statistics
            ...
            if (server_1.status == IDLE) { // server is idle
                ...
                current_departure_time.value = clk.time + (*current_process).service_time;
    ...
    cout << "Average service time = " << area_under_b * 1.0 / double(processes.size()) << " min\n";

    cout << "Average waiting time = " << total_delay_time / double(processes.size()) << " min\n";

    cout << "Average time customer spends in the system = "
         << (total_delay_time + area_under_b) / double(processes.size()) << " min\n";

    cout << "Throughput = " << double(processes.size()) / clk.time << " process/min\n";
